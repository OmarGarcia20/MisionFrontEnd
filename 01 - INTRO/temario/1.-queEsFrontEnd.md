...
- Fase 1: Requerimientos --> Fase donde se presentan las necesidades de la aplicación.
    +Tomar en cuenta que es los proporciona el cliente (nosotros podemos aportar ideas)
    +Siempre analizar a quienes va dirigid@ nuestra aplicacion (user/persona)
        +Ejemplos de los datos de la user persona son edad,empleo,hobbies,red social referida, sexo, etc
        +Este analisis nos ayudará con el diseño de la aplicacion(tipo de letra, color, redes sociales utilizadas, etc)
    +Hacer nuestra marca memorable
